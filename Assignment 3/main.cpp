...
double findMin(std::vector<int>& indices, int k, std::vector<double>& weight, double& min) {
    min = weight[indices[0]];
    int minIdx = 0;

    for (int i = 1; i < k; ++i) {
        if (weight[indices[i]] < min) {
            min = weight[indices[i]];
            minIdx = i;
        }
    }
...
void createRestrictedCandidateList(const int &n, std::vector<double>& weight, std::vector<int>& occupied, int candNumber, std::vector<int>& restrictedCandidateList) {
    int i = 0;
    int k = 0;
    int minIdx;
    double min;
    ...
    minIdx = findMin(restrictedCandidateList, candNumber, weight, min);

    for (; i < n; ++i) {
        if (!occupied[i] && weight[i] > min) {
            restrictedCandidateList[minIdx] = i;
            minIdx = findMin(restrictedCandidateList, candNumber, weight, min);
        }
    }
}
...
void GRASP(const int &n, const int &m, std::vector<int>& x, std::vector<int>& c,
    ...
    //start algo
    while (nassigned < n) {
        getWeight(n, m, c, a, b, occupied, weight);

        candNumber = ceil(alpha * (n - nassigned));
        createRestrictedCandidateList(n, weight, occupied, candNumber, restrictedCandList);
...
double metaheuristic(const int &n, const int &m, Solution& x, std::vector<int>& c,
    ...
    std::vector<int> solution_improvement(n);
    ...
    solutions.push_back(std::pair(improved_solution, computeSolutionObjective(n, c, improved_solution)));
...
int main(int argc, char **argv) {
    ...
    for (auto filename: FILES){
        ...
        if (readInstance(filename.insert(0,"../Files/"), c, a, b, n, m, q, opt)) {
            ...
            std::cout<<metaheuristic(n,m,improved_solution,c,a,b)<<"\t"<<computeSolutionObjective(n,c,improved_solution)<<std::endl;
//            int new_objective;
